    ...
    public async getOwnedGames(): Promise<OwnedSteamGame[]> {
        ...
        try {
            ...
            for (const userId of userDirs) {
                ...
                const localConfigPath = path.join(userdataRoot, userId, 'config', 'localconfig.vdf');
                        ...
                        if (apps) {
                            for (const [appIdStr, appData] of Object.entries(apps)) {
                                ...
                                if (ownedGamesMap.has(appId)) {
                                    ...
                                    existing.playtime_forever = Math.max(existing.playtime_forever, playtime);
        ...
        console.log(`[SteamLibrary] Found ${games.length} unique owned games from local config`);
        return games;
    }
}
